...
func load_game(path: String) -> void:
	...
	var loaded_env_changes: Array = save_manager.get_value("env_changes")
	GameState.env_changes.assign(loaded_env_changes if loaded_env_changes else [])
...
func on_env_changed(node: Node2D) -> void:
	GameState.env_changed(node)
	update_save()
