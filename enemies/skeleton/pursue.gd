...
func state_physics_process(_delta: float) -> void:
	...
	if (distance_to_player > character.stats.max_chase_distance):
		emit_change_state("patrol")
	elif (distance_to_player > character.stats.min_chase_distance):
		character.velocity.x = direction_to_player * character.stats.chase_speed
	else:
